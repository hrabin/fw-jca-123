...
int aes_setkey_dec( aes_context *ctx, const unsigned char *key, int keysize )
{
	...
	*RK++ = *SK++;
	...
	memset( &cty, 0, sizeof( aes_context ) );//tohle je tu jen z bezpecnostniho duvodu, aby ve stacku nezustal klic
...
void aes_crypt_ecb32( aes_context *ctx,
					int mode,
					u32 input[4],
					u32 output[4] )
{
	...
	if( padlock_supports( PADLOCK_ACE ) )
	{
		if( padlock_xcryptecb( ctx, mode, (u8 *)input, (u8 *)output ) == 0 )
			return;
	}
#endif
					...
					unsigned char *output )
{
	u32 _temp[4];
	int i;
	bool u32_optimise=false;

	#define	temp ((u8 *)_temp)
	...
	if( mode == AES_DECRYPT )
	{
		while( length > 0 )
		{
			if (u32_optimise)
			{
				for( i = 0; i < 4; i++ )
					_temp[i] = ((u32 *)input)[i];
				aes_crypt_ecb32( ctx, mode, (u32 *)input, (u32 *)output );
				for( i = 0; i < 4; i++ )
				{
					((u32 *)output)[i] = (u32)( ((u32 *)output)[i] ^ ((u32 *)iv)[i] );
					((u32 *)iv)[i] = _temp[i];
				}
			}
			else
	...
	else
	{
		while( length > 0 )
		{
			if (u32_optimise)
			{
				for( i = 0; i < 4; i++ )
				{
					((u32 *)output)[i] = (u32)( ((u32 *)input)[i] ^ ((u32 *)iv)[i] );
				}
				aes_crypt_ecb32( ctx, mode, (u32 *)output, (u32 *)output );
				for( i = 0; i < 4; i++ )
					((u32 *)iv)[i] = ((u32 *)output)[i];
			}
			else
			{
				for( i = 0; i < 16; i++ )
